...
static void InitOMXParams(T *params) {
    ...
    ALOGV("IGOR_DEBUG_TEST_AC3_DECODER_CLASS");
    ALOGW("IGOR_DEBUG_TEST_AC3_DECODER_CLASS");
...
SoftAC3::SoftAC3(
    ...
    CHECK(!strcmp(name, "OMX.igor.ac3.decoder"));
...
void SoftAC3::onQueueFilled(OMX_U32 /* portIndex */) {
    ...
    while (!inQueue.empty() && !outQueue.empty()) {
        ...
        int n = 2;//mSignalledError ? 0 : DecodeGSM(mGsm,
...
// static
/*int SoftAC3::DecodeGSM(gsm handle,
...
void SoftAC3::onPortFlushCompleted(OMX_U32) {
  /*  if (portIndex == 0) {
        gsm_destroy(mGsm);
        mGsm = gsm_create();
        int msopt = 1;
